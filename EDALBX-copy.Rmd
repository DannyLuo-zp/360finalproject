...
#change to numeric
for(i in 1:50){
  data_reduced[,i]=as.numeric(unlist(data_reduced[,i]))-1
}
...
score_data_final1<-na.omit(score_data[,51:55])

```

```{r}
score_data_final <- scale(score_data_final1)
score_data_final<-as.data.frame(score_data_final)
nrow(score_data_final)
```
...
```{r}
data = sample_n(score_data_final,10000)
data = as.matrix(data)
head(data)
```
...
#Calculating overlaps. COuld find an alternative solution
...
```{r}
Overlap<-matrix( 0,nrow = 12, ncol = 2)
...
#This baby calculates the posterior
